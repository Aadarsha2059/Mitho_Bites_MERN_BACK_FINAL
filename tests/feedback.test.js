...
describe("Feedback API", () => {
  ...
  test("should return empty array for feedback search with no results", async () => {
    const res = await request(app).get("/api/admin/feedback").query({ search: "no_such_feedback_12345" });
    expect(res.statusCode).toBe(200);
    expect(res.body.success).toBe(true);
    expect(Array.isArray(res.body.data)).toBe(true);
  ...
  test("should return 404 when updating non-existent feedback", async () => {
    const fakeId = new mongoose.Types.ObjectId();
    const res = await request(app).put(`/api/admin/feedback/${fakeId}`).send({ rating: 5, comment: "Updated!" });
    expect(res.statusCode).toBe(404);
    expect(res.body.success).toBe(false);
